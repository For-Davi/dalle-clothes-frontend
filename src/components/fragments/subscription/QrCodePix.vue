...
import { createSuccess, createErrorData } from 'src/composables/CreateNotify';
...
const getQRCode = async (subscriptionID: number) => {
...
const copyPayload = async () => {
  try {
    ...
    createSuccess('Chave copiada');
  } catch (e) {
    console.error('Erro ao copiar:', e);
    createErrorData('Erro ao copiar chave');
  }
};

onMounted(async () => {
  await getQRCode(props.subscriptionID);
});
</script>
  ...
  <section>
    <div>
      ...
      >
        ...
        >
        <q-btn label="Copiar chave" color="primary" @click="copyPayload()" no-caps outline />
      </div>
    </div>
  </section>
</template>
