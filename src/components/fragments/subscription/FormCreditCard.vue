...
const model = defineModel<IVMPaymentSubscriptionCreditCard>({
  default: () => ({
    creditCard: {
      holderName: '',
      number: '',
...
const formattedPhone = computed({
  get() {
    const phone = (model.value.creditCardHolderInfo.phone || '').replace(/\D/g, '');
  ...
  async () => {
    ...
    if (allowSearchCep.value) {
      if (model.value.creditCardHolderInfo.postalCode.trim().length === 8) {
        loading.value = true;
        const response = await searchCep(model.value.creditCardHolderInfo.postalCode);
...
<template>
  <section>
    <q-form class="q-gutter-y-xs column flex justify-center">
      <div>
        <TitlePage title="Informações do cartão de crédito" icon="credit_card" />
      </div>
      <q-input
        label="Nome"
      ...
      <div class="q-mt-md">
        <TitlePage title="Informações do titular do cartão de crédito" icon="person" />
      </div>
      <q-input
        label="Nome"
