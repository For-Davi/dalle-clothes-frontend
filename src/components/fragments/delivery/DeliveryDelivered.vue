...
const fetchDeliveredDelieries = async () => {
  await useDeliveryStore().getDeliveries('delivered');
};

onMounted(async () => {
  await fetchDeliveredDelieries();
...
<template>
  <main>
    <section class="q-mt-md q-px-sm">
      ...
      <DeliveriesList :filter="search" status="delivered" :list-deliveries="listDelivery" />
    </section>
  </main>
</template>
